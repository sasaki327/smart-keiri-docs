...
ITツールに不慣れな方でも「迷わず・見落とさず」直感的に操作できることを最優先とし、徹底的な視認性を追求した **OJI-UX（Optimal Judicious Interface）** を採用。最新世代の **Google Gemini**（AIモデル）による領収書解析（OCR）と、国税庁API連携によるインボイス監査を統合し、経理実務の正確性をシステムレベルで担保します。

### コンセプト：プロの仕事を、ストレスフリーに。
複雑な設定は不要です。**「経理担当者の負担を最小化し、経営判断を加速させる」** 現場主義の設計により、月末の事務作業を確実に、そして美しく終わらせるための仕組みを提供します。
...
独自の「安心」と「実利」を追求しています。

### 1. 「専用環境」による圧倒的な安心感
共有サーバー（マルチテナント）ではなく、物理的に独立したサーバー環境を構築します。機密性の高い経理データを他社と混在させることなく、安全に管理。独自のカスタマイズも柔軟に対応可能です。
...
- **監査ナビゲーション**: AIが読み取り結果を評価し、「✨ AI自動入力」や「⚠️ 要確認」といった定性的なバッジで案内。公的データ検証後はノイズを抑えるスマートな表示制御を搭載。
...
### v1.2.2 (2026/03/15)
- **電子帳簿保存法への適合性強化 (Amount Search Support)**: 申請一覧に「金額の範囲検索」を追加。法的に求められる日付・金額・取引先による検索3条件を完全に網羅。
- **インテリジェント・フィルターの強化**: 収益管理がオフの場合、検索条件のカテゴリー候補から収益項目を自動除外する連動ロジックを実装。
- **品質・テスト環境の整備**: データベース制約の変更（カテゴリー必須化）に伴うテストセットの不備を解消し、システム全体の信頼性を向上。

### v1.2.1 (2026/02/28)
- **UI/UX革新 (Qualitative AI Badges)**: パーセンテージ表示を廃止し、定性的な「✨ AI自動入力」バッジへ刷新。国税庁API検証済み項目のAIバッジ非表示化など、徹底したノイズ削減を実施。
- **非課税・対象外支出への完全対応**: 医療費（保険診療）、印紙代、保険料などの非課税項目を正確に抽出・管理できる「対象外・非課税額」フィールドを新設。AI解析ロジックもこれに対応。
- **システム安定性の向上**: セッション管理の改善によるログアウトフリーズ解消、および宿泊費等におけるカテゴリ・インボイス判定の厳格化。
...
SMART KEIRI は、中小企業向け業務SaaSの検証・提供を目的としたプロジェクトです。本リポジトリではサービス概要・思想・リリース情報のみを公開しています。

---

## 📄 ライセンス

このプロジェクトはデモンストレーション目的で作成されています。セールスは行っていません。
ソースコードの著作権は開発者に帰属します。
