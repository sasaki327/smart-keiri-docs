...
- **SMK Premium Modal**: 確認ダイアログをグラスモーフィズムデザインへ刷新。操作に応じた色彩同期（申請：緑, 削除：赤, AI：紫）を導入し、直感的でプレミアムな操作感を提供。
- **インテリジェント UI 制御**: 有効化された機能（売上管理等）に応じて、ナビゲーションメニューなどの UI 要素を動的に制御。
...
### v1.2.0 (2026/02/21)
- **SMK プレミアム・デザイン導入**: ボタン全体に高品質なソリッドカラーとシャドウを適用。確認ダイアログをグラスモーフィズムデザインの `smkConfirm` へ刷新し、プレミアムな操作感を実現。
- **収益（売上）管理のオプトイン導入**: 設定一つで「支出のみ」から、個人事業主・副業向けの「売上管理」フル機能 UI へ切り替え可能。
- **管理者による直接修正機能**: 承認済み案件でも管理者が内容を最終調整可能に（履歴自動記録・仕訳同期）。
- **インテリジェント UI 制御 & 重複検知強化**: 機能に応じたメニューの動的制御と、AI解析直後の二重申請アラート機能を搭載。
- **ユーザーメニューの最適化**: ヘッダーのドロップダウン化により、モバイルでも崩れないモダンな UI を実現。
